...
export default function CardNew({ exp: data }) {
    const {
        i18n: { language },
    } = useTranslation();
    ...
    return (
        data && (
            <Link
                to={`/${language}/announcements/${data.id}`}
                className="group hover:shadow-xl rounded-md! overflow-hidden border border-border flex flex-col"
            >
                <img
                    ...
                    alt="news"
                />
                <div className="p-4 pb-5 space-y-3 flex flex-col flex-1 justify-between">
